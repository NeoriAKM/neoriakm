[![Top Langs](https://github-stats-extended.vercel.app/api/top-langs/?username=NeoriAKM&layout=compact&theme=monokai&hide=html,css&card_width=400)](https://github.com/stats-organization/github-stats-extended)
[![NeUI Repo](https://github-stats-extended.vercel.app/api/pin/?username=NeoriAKM&repo=neui&theme=monokai)](https://github.com/NeoriAKM/neui)


# EN
## Hello!
My name is Akhmed. Я живу в России (I live in Russia), I'm 13 years old, and I love Linux :)

### My devices
**Main PC:** VivoBook Max 15 (Model X541UV-XO241T) — a budget laptop from 2016 with:
- 500 GB HDD
- 8 GB RAM
- Intel Core i5-6198DU 2.8 GHz (2 cores)
- Intel HD Graphics 510 + discrete NVIDIA GeForce 920MX
- 15" TN screen, 1366×768, 60 Hz
- OS: EndeavourOS (Linux Distribution on base Arch Linux)
Just search in google this model

**Phone:** Redmi Note 8 (4/64 GB, Snapdragon 665)

### Who am I
I'm a junior developer. I write in VS Code, and when I'm in the tty — I use Vim.

I know **Python, C, Lua and HTML&CSS**.

[![](https://skillicons.dev/icons?i=python,c,lua,html,linux,git,vim,bash,arch&theme=dark)](https://skillicons.dev)

My main and only 2 projects now is [NeoUtils](https://github.com/neoriakm/neoutils) & [NeUI](https://github.com/neoriakm/neui)

### My tools:
- **Terminal** : Alacritty
- **Shell**    : fish
- **DE/WM**    : XFCE4, sometimes KDE Plasma

---
---
# RU
## Привет!
Меня зовут Ахмед. Я живу в России, мне 13 лет, и я люблю линукс :)

### Мои девайсы
**Компьютер**: VivoBook Max 15 (Модель X541UV-XO241T) - бюджетный ноутбук 2016 года, характеристики:
- **Экран**: 1366x768, 60 Гц, TN
- **ПЗУ**: 500 ГБ HDD
- **ОЗУ**: 8 ГБ
- **Процессор**: Intel Core i5-6198DU 2.8 ГГц (2 ядра)
- **Видеокарта**: Intel HD Graphics 510 + дискретная NVIDIA GeForce 920MX
- **ОС**: EndeavourOS (Линукс дистрибутив на базе Arch Linux)

**Телефон**: Redmi Note 8 (4/64 ГБ, Snapdragon 665)

### Кто я
Я junior разработчик. Я пишу в VS Code, а когда в tty - использую Vim.

Знаю **Python, C, Lua и HTML**.

[![](https://skillicons.dev/icons?i=python,c,lua,html,linux,git,vim,bash,arch&theme=dark)](https://skillicons.dev)

Основные и единственные мои проекты - это [NeoUtils](https://github.com/neoriakm/neoutils) & [NeUI](https://github.com/neoriakm/neui)

### Мои инструменты
- **Терминал**: alacritty
- **Командная оболочка**: fish
- **Среда рабочего стола**: XFCE4, иногда KDE Plasma


---
Спасибо за то что дочитал / Thanks for reading!
